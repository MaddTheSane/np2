...
static const char str_dirname[] = "Ì§²Ù‚ÌêŠ";
static const char str_parent[] = "©";
static const char str_filename[] = "Ì§²Ù–¼";
static const char str_filetype[] = "Ì§²Ù‚ÌŽí—Þ";
...
const char	*title;
	...
	soundmng_disable(SNDPROC_MENU);
	ZeroMemory(&filesel, sizeof(filesel));
	if ((def) && (def[0])) {
		file_cpyname(filesel.path, def, sizeof(filesel.path));
	}
	else {
		file_cpyname(filesel.path, modulefile, sizeof(filesel.path));
		file_cutname(filesel.path);
	}
	title = NULL;
	if (prm) {
