...
static void cat_hex8(UNASM_T *unasm_t, BYTE value) {
	...
	if ((value >= 0x20) && (value < 0x7f)) {
		SPRINTF(unasm_t->extend, ";'%c'", value);
	}
}
...
static void cat_hex16(char *str, UINT16 value) {
...
static void cat_mem16(char *str, UINT16 value, PREFIX_T *fix) {
...
static int cat_ea(BYTE *bincode, PREFIX_T *fix, UNASM_T *unasm_t) {
	...
	if ((bincode[1] & 0xc7) != 0x06) {
		...
		switch(bincode[1] & 0xc0) {
			...
			case 0x80:
				strcat(unasm_t->operand, "+");
				cat_hex16(unasm_t->operand, *(UINT16 *)(bincode + 2));
				ret = 2;
				break;
		}
	}
	...
	if ((bincode) && (fix)) {
		...
		switch(mnemonic[c].type) {
			...
			case OPERAND_EA16_IMM16:
				...
				cat_cnm(una.operand);
				cat_hex16(una.operand, *(UINT16 *)(bincode + pos + 1));
				ret += pos + 2;
				break;
			...
			case OPERAND_AL_MEM:
				...
				cat_cnm(una.operand);
				cat_mem16(una.operand, *(UINT16 *)(bincode + 1), fix);
				ret += 2;
				break;
			...
			case OPERAND_AX_IMM16:
				strcpy(una.operand, str_ax);
				cat_cnm(una.operand);
				cat_hex16(una.operand, *(UINT16 *)(bincode + 1));
				ret += 2;
				break;
			...
			case OPERAND_IMM_FAR:
				cat_hex16(una.operand, *(UINT16 *)(bincode + 3));
				strcat(una.operand, ":");
				cat_hex16(una.operand, *(UINT16 *)(bincode + 1));
				ret += 4;
				break;
