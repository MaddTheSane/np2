...
#include "fdd_mtr.res"
#else	/* defined(SUPPORT_SWSEEKSND) */
#include "soundmng.h"
#endif	/* defined(SUPPORT_SWSEEKSND) */
...
void fddmtrsnd_initialize(UINT rate)
{
	...
	if (np2cfg.MOTORVOL)
	{
		...
		pcmmix_regist(&mtrsnd.snd.trk[0].data, (void *)fddseek, sizeof(fddseek), rate);
		mtrsnd.snd.trk[0].flag = PMIXFLAG_L | PMIXFLAG_R | PMIXFLAG_LOOP;
		mtrsnd.snd.trk[0].volume = (np2cfg.MOTORVOL << 12) / 100;
		pcmmix_regist(&mtrsnd.snd.trk[1].data, (void *)fddseek1, sizeof(fddseek1), rate);
...
void fddmtr_seek(REG8 drv, REG8 c, UINT size)
{
	...
	if (!np2cfg.MOTOR)
	{
		...
		{
			fddmtr.busy = 1;
			nevent_set(NEVENT_FDBIOSBUSY, size * pccore.multiple, fdbiosout, NEVENT_ABSOLUTE);
		}
		return;
	}
	...
	{
		fddmtr.busy = 1;
		nevent_setbyms(NEVENT_FDBIOSBUSY, waitcnt, fdbiosout, NEVENT_ABSOLUTE);
	}
	(void)drv;
}
