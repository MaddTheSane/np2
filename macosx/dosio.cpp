...
short file_rename(const char *existpath, const char *newpath) {
	...
	mkstr255(fname, existpath);
	FSMakeFSSpec(0, 0, fname, &fsSrc);

	mkstr255(fname, newpath);
	FSMakeFSSpec(0, 0, fname, &fsDest);

	iErr = FSpCatMove(&fsSrc, &fsDest);
	if (iErr == noErr) {
		return 0;
	} else {
		return -1;
