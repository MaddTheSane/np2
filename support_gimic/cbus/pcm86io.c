...
static REG8 IOINPCALL pcm86_ia466(UINT port)
{
	...
	if (g_pcm86.nVirtualCount >= PCM86_LOGICALBUF)		/* バッファフル */
	{
		ret |= 0x80;
	}
	else if (!g_pcm86.nVirtualCount)					/* バッファ０ */
	{
		ret |= 0x40;
	}
	(void)port;
